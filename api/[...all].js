...
async function sendContactMessage(rawInput) {
  ...
  if (!apiKey || !recipients || recipients.length === 0) {
    ...
    throw new TRPCError3({ code: "INTERNAL_SERVER_ERROR", message: "Email service unavailable" });
  }
  ...
  if (!response.ok) {
    ...
    throw new TRPCError3({ code: "BAD_GATEWAY", message: "Email service rejected the message" });
  }
  return { success: true };
}
...
var appRouter = router({
  ...
  contact: router({
    submit: publicProcedure.input(contactInputSchema).mutation(({ input }) => sendContactMessage(input))
  })
});
